...
- Color:
  - Adaptive window backgrounds, labels, separators, and controls remain native system colors.
  - `Brand Indigo 950` `#0A1530` and `Brand Indigo 800` `#1B3262` form the app-icon field and may be used sparingly for durable branded surfaces.
  - `Flow Mint 300` `#9AF0C8` and `Flow Mint 500` `#42D1B0` form the primary brand accent. Mint denotes active, healthy local flow; text must still name the state.
  ...
  - Do not place body text directly on mint. Maintain at least WCAG AA contrast for text and essential glyphs in both appearances.
- Typography: use the system/SF family exclusively. Large Title for product-level headings, Title 3 for panel headings, Headline for section/card titles, Callout for primary explanatory copy, Caption for provenance and privacy detail. Use monospaced system text only for hashes, versions, paths, or diagnostic identifiers.
- Spacing/layout rhythm: 4 pt base rhythm; preferred steps are 8, 12, 16, 24, and 32 pt. Settings/onboarding use 22–24 pt section separation and 16 pt card insets. Keep one strong alignment edge per surface.
- Shape/radius/elevation: 10 pt result containers, 12 pt setup/settings cards, and a continuous capsule for the Flow Bar. Its soft shadow must fully fade inside the transparent panel bounds so no rectangular clipping edge is visible. Ordinary settings cards rely on system fill rather than elevation.
...
- Token/component ownership: system semantic values remain owned by SwiftUI/AppKit. Brand color values and layout rules are owned by this file. Do not add a theme framework or dependency; introduce code tokens only after repeated implementation use demonstrates value.
...
- Contrast/readability: support Light, Dark, Increase Contrast, and Reduce Transparency system settings. Never rely on mint/green/orange/red without a title, symbol, or status string. Preserve text selection in fallback results.
...
- Layout adaptations:
  - Settings scroll vertically with a 590 pt minimum content width; onboarding scrolls with a 610 pt minimum.
...
- Design-token constraints: prefer system semantic values. Any introduced brand token must match the values in Visual language and be used consistently in Light/Dark appearance.
...
- Test/screenshot expectations:
  ...
  - For future UI changes, capture onboarding, settings, and Flow Bar active/error states in Light/Dark appearance; include transparent-corner, Increase Contrast, Reduce Transparency, and Reduce Motion checks for release-facing changes.
